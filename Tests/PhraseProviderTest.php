...
class PhraseProviderTest extends ProviderTestCase
{
    ...
    public function toStringProvider(): iterable
    {
        yield [
            ...
            ]), $this->getLoader(), $this->getLogger(), $this->getDefaultLocale(), 'api.phrase.com'),
            'phrase://api.phrase.com',
        ];

        yield [
            $this->createProvider($this->getClient()->withOptions([
                'base_uri' => 'https://domain.api.phrase.com/api/v2/projects/1/',
                'auth_bearer' => 'API_TOKEN',
            ]), $this->getLoader(), $this->getLogger(), $this->getDefaultLocale(), 'domain.api.phrase.com'),
            'phrase://domain.api.phrase.com',
        ];

        yield [
            $this->createProvider($this->getClient()->withOptions([
                'base_uri' => 'https://api.phrase.com:99/api/v2/projects/1/',
                'auth_bearer' => 'API_TOKEN',
            ]), $this->getLoader(), $this->getLogger(), $this->getDefaultLocale(), 'api.phrase.com:99'),
            'phrase://api.phrase.com:99',
        ];
    }

    public function disabled_testCompleteWriteProcessAddFiles()
    {
...
XLIFF;
        ...
        $responses = [
            'listFiles' => function (string $method, string $url, array $options = []): ResponseInterface {
                ...
                $this->assertSame('https://api.phrase.com/api/v2/projects/1/locales/fr/download?file_format=symfony_xliff', $url);
            ...
            'addStorage' => function (string $method, string $url, array $options = []) use ($expectedMessagesFileContent): ResponseInterface {
                ...
                $this->assertSame('https://api.phrase.com/api/v2/storages', $url);
                $this->assertSame('Content-Type: application/octet-stream', $options['normalized_headers']['content-type'][0]);
                $this->assertSame('Phrase-API-FileName: messages.xlf', $options['normalized_headers']['phrase-api-filename'][0]);
            ...
            'addStorage2' => function (string $method, string $url, array $options = []) use ($expectedValidatorsFileContent): ResponseInterface {
                ...
                $this->assertSame('https://api.phrase.com/api/v2/storages', $url);
                $this->assertSame('Content-Type: application/octet-stream', $options['normalized_headers']['content-type'][0]);
                $this->assertSame('Phrase-API-FileName: validators.xlf', $options['normalized_headers']['phrase-api-filename'][0]);
        ...
        $provider = $this->createProvider((new MockHttpClient($responses))->withOptions([
            'base_uri' => 'https://api.phrase.com/api/v2/projects/1/',
            'auth_bearer' => 'API_TOKEN',
        ]), $this->getLoader(), $this->getLogger(), $this->getDefaultLocale(), 'api.phrase.com/api/v2/projects/1/');

        $provider->write($translatorBag);
    }

    public function disabled_testCompleteWriteProcessUpdateFiles()
    {
        $this->xliffFileDumper = new XliffFileDumper();
...
XLIFF;
        ...
        $responses = [
            ...
            'UpdateFile' => function (string $method, string $url, array $options = []): ResponseInterface {
                ...
                $this->assertSame('https://api.phrase.com/api/v2/projects/1/locales/fr/download?file_format=symfony_xliff/12', $url);
    ...
    public function disabled_testCompleteWriteProcessAddFileAndUploadTranslations()
    {
        $this->xliffFileDumper = new XliffFileDumper();
...
XLIFF;
        ...
        $responses = [
            ...
            'UploadTranslations' => function (string $method, string $url, array $options = []): ResponseInterface {
                $this->assertSame('POST', $method);
                $this->assertSame('https://api.phrase.com/api/v2/projects/1/translations/fr', $url);
    ...
    public function testReadForOneLocaleAndOneDomain(string $locale, string $domain, string $responseContent, TranslatorBag $expectedTranslatorBag)
    {
        $responses = [
            ...
            'exportProjectTranslations' => function (string $method, string $url, array $options = []): ResponseInterface {
                $this->assertSame('POST', $method);
                $this->assertSame('https://api.phrase.com/api/v2/projects/1/translations/exports', $url);
        ...
        $phraseProvider = $this->createProvider((new MockHttpClient($responses))->withOptions([
            'base_uri' => 'https://api.phrase.com/api/v2/projects/1/',
            'auth_bearer' => 'API_TOKEN',
        ]), $this->getLoader(), $this->getLogger(), $this->getDefaultLocale(), 'api.phrase.com/api/v2');

        $translatorBag = $phraseProvider->read([$domain], [$locale]);
...
<?xml version="1.0" encoding="UTF-8"?>
<xliff xmlns="urn:oasis:names:tc:xliff:document:1.2" version="1.2">
  <file source-language="en" target-language="fr" datatype="database" tool-id="phrase">
    <header>
      <tool tool-id="phrase" tool-name="Phrase" tool-version="1.0.25 20201211-1" tool-company="Phrase"/>
    </header>
    <body>
      <trans-unit id="phrase:5fd89b853ee27904dd6c5f67" resname="index.hello" datatype="plaintext">
        <source>index.hello</source>
        <target state="translated">Bonjour</target>
      </trans-unit>
      <trans-unit id="phrase:5fd89b8542e5aa5cc27457e2" resname="index.greetings" datatype="plaintext" extradata="phrase:format=icu">
...
XLIFF
    ...
    public function testReadForDefaultLocaleAndOneDomain(string $locale, string $domain, string $responseContent, TranslatorBag $expectedTranslatorBag)
    {
        $responses = [
            'listFiles' => function (string $method, string $url): ResponseInterface {
                $this->assertSame('GET', $method);
                $this->assertSame('https://api.phrase.com/api/v2/projects/1/locales/en/download?file_format=symfony_xliff', $url);
            ...
            'downloadSource' => function (string $method, string $url): ResponseInterface {
                $this->assertSame('GET', $method);
                $this->assertSame('https://api.phrase.com/api/v2/projects/1/locales/fr/download?file_format=symfony_xliff/12/download', $url);
...
<?xml version="1.0" encoding="UTF-8"?>
<xliff xmlns="urn:oasis:names:tc:xliff:document:1.2" version="1.2">
  <file source-language="en" target-language="fr" datatype="plaintext" tool-id="phrase">
    <header>
      <tool tool-id="symfony" tool-name="Symfony"/>
    </header>
    <body>
...
XLIFF
    ...
    public function disabled_testDelete()
    {
        $responses = [
            'listFiles' => function (string $method, string $url): ResponseInterface {
                $this->assertSame('GET', $method);
            ...
            'listStrings1' => function (string $method, string $url): ResponseInterface {
                $this->assertSame('GET', $method);
                $this->assertSame('https://api.phrase.com/api/v2/projects/1/strings?fileId=12&limit=500&offset=0', $url);
            ...
            'listStrings2' => function (string $method, string $url): ResponseInterface {
                $this->assertSame('GET', $method);
                $this->assertSame('https://api.phrase.com/api/v2/projects/1/strings?fileId=12&limit=500&offset=500', $url);
            ...
            'deleteString1' => function (string $method, string $url): ResponseInterface {
                ...
                $this->assertSame('https://api.phrase.com/api/v2/projects/1/strings/1', $url);

                return new MockResponse('', ['http_code' => 204]);
            },
            'deleteString2' => function (string $method, string $url): ResponseInterface {
                $this->assertSame('DELETE', $method);
                $this->assertSame('https://api.phrase.com/api/v2/projects/1/strings/2', $url);
